...
int main() {
  ...
  if ((user_input = getchar()) == 's' || (user_input = getchar()) == 'S') {
    while ((user_input != 'q' || user_input == 'Q') &&
      ...
      printf("x: %d y: %d x_v: %d y_v: %d\n", x_ball, y_ball, vector_x,
             vector_y);
      user_input = getchar();
      if (user_input == 'a')
...
void print_score(int p1, int p2) { printf("%d < GAME SCORE > %d\n", p1, p2); }
